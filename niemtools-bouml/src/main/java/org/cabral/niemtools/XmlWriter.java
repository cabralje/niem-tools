                    ...
                    try (FileWriter fw = new FileWriter(file)) {
                        ...
                        if (codeList.contains(NiemModel.CODELIST_DELIMITER)) {
                            ...
                            for (String code : codes) {
                                ...
                                fw.write("<Row><Value ColumnRef=\"" + CODELIST_CODE + "\"><SimpleValue>"
                                        + pairs[0].trim()
                                        //	+ code
                                        + "</SimpleValue></Value>");
                                if (pairs.length > 1) //if (codeDescription != null && !codeDescription.equals(""))
            ...
            for (UmlItem item2 : port.children()) {
                ...
                if (params != null) {
                    ...
                    for (UmlParameter param : params) {
                        ...
                        if (NiemUmlClass.isNiemUml(inputType)) {
                            Log.trace("exportWSDL: error - no type for input message for " + operationName);
                            continue;
                        }
                        String inputMessage = null;
                        if (inputType != null)
                            inputMessage = inputType.propertyValue(NiemUmlClass.NIEM_STEREOTYPE_XPATH);
                ...
                if (outputType == null || !NiemUmlClass.isNiemUml(outputType)) {
...
//					if (outputTypeName != null)
    ...
    String exportXmlTypeSchema(UmlClass type) {
        ...
        if (NiemUmlClass.isEnumeration(type)) {
        ...
        } else if (NiemUmlClass.isEnumeration(baseType)) {
        ...
        if (isComplexContent == false) //if (codeList != null && codeList.contains(NiemModel.CODELIST_DELIMITER)) {
        //	for (String code : codeList.split(NiemModel.CODELIST_DELIMITER)) {
        {
            if (NiemUmlClass.isEnumeration(type)) {
                for (UmlItem item : type.children()) {
                    ...
                    if (NiemUmlClass.isFacet(attribute))
                        continue;
        ...
        //if (isComplexType) {
        if (isComplexContent)
            typeSchema += "<xs:complexContent>" + "<xs:extension base=\"" + baseTypeName + "\">"
                    + "<xs:sequence>" + String.join("", xmlElementsInType) + "</xs:sequence>"
                    + "</xs:extension>" + String.join("", xmlAttributesInType) + "</xs:complexContent>";
        else
            ...
            if (!xmlEnumerations.isEmpty()) {
                ...
                typeSchema += "<xs:simpleContent>" + "<xs:extension base=\"" + baseTypeName + "\">"
                        + String.join("", xmlAttributesInType) + "" + "</xs:extension>";
            }
            typeSchema += "</xs:simpleContent>\n";
        typeSchema += (isComplexType) ? "</xs:complexType>" : "</xs:simpleType>";
        return typeSchema;
    ...
    static public String xmlNs(String prefix, String value) {
        if (prefix.equals(""))
            return " " + XMLConstants.XMLNS_ATTRIBUTE + "=\"" + value + "\""; 
        else
            return " " + XMLConstants.XMLNS_ATTRIBUTE + ":" + prefix + "=\"" + value + "\"";
    }
}
